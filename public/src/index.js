...
btnIniciar.onclick = (e) => {
  ...
  let posiciones = highScore
    .map((hs) => nuevoHighScore(hs.nombre, hs.puntaje))
    .join("");
...
let dibujarCabeza = (x, y) => {
  ...
  ctx.fillRect(
    x + dospixels * (comidaALaDerecha ? 1.5 : 1),
    y + dospixels * (comidaAbajo ? 1.5 : 1),
    dospixels / 2,
    dospixels / 2
  );
  ctx.fillRect(
    x + dospixels * (comidaALaDerecha ? 3.5 : 3),
    y + dospixels * (comidaAbajo ? 1.5 : 1),
    dospixels / 2,
    dospixels / 2
  );
...
let choco = () => {
  ...
  let seComioLaPared = controles.paredes.some(
    (pared) => head.x === pared.x && head.y === pared.y
  );
...
let paredesALaMitad = () => {
  ...
  for (y = 0; y < DIMENSION_CANVAS / 2; y += ANCHO_ALTO_VIBORITA) {
    ...
    controles.paredes.push({
      x: DIMENSION_CANVAS - ANCHO_ALTO_VIBORITA,
      y: y + DIMENSION_CANVAS / 2,
    });
...
let pausarResumir = () => {
  if (controles.pausable) {
    ...
    if (controles.pausa) {
      ...
      if(controles.movimientoParedes) controles.movimientoParedes.pause();
      controles.animaciones.forEach((animacion) => {
        animacion.pause();
      });
    } else {
      ocultar(pantallaPausa);
      if(controles.movimientoParedes) controles.movimientoParedes.resume();
      controles.animaciones.forEach((animacion) => {
        animacion.resume();
      });
    }
...
let retroceder = () => {
  ...
  let pasoCuatro = new Timer(() => {
    ...
    controles.cuatroPasosAtras.posiciones[0] =
      controles.cuatroPasosAtras.posiciones[
        controles.cuatroPasosAtras.posiciones.length - 1
      ];
    console.log(
      "posiciones antes de modificar el lenght:",
      JSON.parse(JSON.stringify(controles.cuatroPasosAtras.posiciones))
    );
    controles.cuatroPasosAtras.posiciones.length = 1;
    controles.cuatroPasosAtras.direcciones[0] =
      controles.cuatroPasosAtras.direcciones[
        controles.cuatroPasosAtras.direcciones.length - 1
      ];
      console.log(
        "direcciones antes de mod el lenght:",
        JSON.parse(JSON.stringify(controles.cuatroPasosAtras.direcciones))
        );
        controles.cuatroPasosAtras.direcciones.length = 1;
        console.log(
          "posiciones despues de mod el length:",
          JSON.parse(JSON.stringify(controles.cuatroPasosAtras.posiciones))
    );
    console.log(
      "direcciones después de mod:",
      JSON.parse(JSON.stringify(controles.cuatroPasosAtras.direcciones))
    );
  }, 4000);
...
let paredesPorLado = () => {
  return {
    paredesIzquierda: controles.paredes.filter(
      (pared) =>
        pared.x === 0 && pared.y !== DIMENSION_CANVAS - ANCHO_ALTO_VIBORITA
    ),
    paredesDerecha: controles.paredes.filter(
      (pared) =>
        pared.x === DIMENSION_CANVAS - ANCHO_ALTO_VIBORITA && pared.y !== 0
    ),
    paredesAbajo: controles.paredes.filter(
      (pared) =>
        pared.y === DIMENSION_CANVAS - ANCHO_ALTO_VIBORITA &&
        pared.x !== DIMENSION_CANVAS - ANCHO_ALTO_VIBORITA
    ),
    paredesArriba: controles.paredes.filter(
      (pared) => pared.y === 0 && pared.x !== 0
    ),
  };
};

let paredesSeMueven = (paredes, paso = 0) => {
  controles.movimientoParedes = null;
  let mov;

  //VAN A SER UN DESCANSO DE 1SEG, 3 PASOS DE 1SEG, UN DESCANSO DE 2SEG, LA VUELTA Y OTRO DESCANSO DE 1 SEG

  //Las paredes de la izquierda se mueven desde arriba hacia abajo y vuelven
  //Las paredes de la derecha se mueven desde abajo hacia arriba y vuelven
  //Las paredes de arriba se mueven desde la derecha hacia la izquierda y vuelven
  //Las paredes de abajo se mueven desde la izquierda hacia la derecha y vuelven
  ...
  switch (paso) {
    case 0:
      ...
      //DESCANSO 1 SEG
      mov = new Timer(() => {
        paredesSeMueven(paredes, ++paso);
      }, 1000);
      controles.movimientoParedes = mov;
      break;
    case 1:
    case 2:
    case 3:
      //PASO 1, 2 Y 3
      mov = new Timer(() => {
        paredes.paredesIzquierda.forEach((pared) => {
          pared.y += ANCHO_ALTO_VIBORITA * 5;
        });
        paredes.paredesDerecha.forEach((pared) => {
          pared.y -= ANCHO_ALTO_VIBORITA * 5;
        });
        paredes.paredesArriba.forEach((pared) => {
          pared.x -= ANCHO_ALTO_VIBORITA * 5;
        });
        paredes.paredesAbajo.forEach((pared) => {
          pared.x += ANCHO_ALTO_VIBORITA * 5;
        });
...
let loop = () => {
  ...
  if (controles.animando) {
    ...
    if (imgAnterior !== imgActual) {
      ...
      console.log(
        "direccion: ",
        JSON.parse(JSON.stringify(controles.direccion))
      );
      console.log(
        "proximadireccion: ",
        JSON.parse(JSON.stringify(controles.proximaDireccion))
      );
      console.log(
        "cuatropasosatras: ",
        JSON.parse(JSON.stringify(controles.cuatroPasosAtras))
      );
      console.log(
        "%c ",
        `font-size:400px; background:url(${imgActual}) no-repeat;`
      );
